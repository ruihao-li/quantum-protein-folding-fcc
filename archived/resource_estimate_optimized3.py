## iteratively estimates resource requirement per sequence length, with an adjustable circuit optimization
## level (set to level 3 here)
...
with open("estimate_op3.tsv", "w+") as outfile:
...
#         print(ansatz)
        ...
        ## define and set optimization level
        optimized_3 = transpile(ansatz, backend=provider.get_backend(backend_name), seed_transpiler=11, optimization_level=3)
    
        
        #iteratively write the resources required per sequence size 
        outfile.write("{}\t{}\t{}\t{}\t{}\t{}\n".format((len(main_chain)), (qubit_op.num_qubits), (len(optimized_3.count_ops())), (optimized_3.depth()), "3", (optimized_3.count_ops())))
    t1 = time()
