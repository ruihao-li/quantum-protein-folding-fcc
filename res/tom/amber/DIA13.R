system("pwd")
setwd("/home/radivot/GH/protein-folding-qc/res/tom/amber/DIA13")
if (0){  # stuff to do just once, to initialize the folder
  system("mkdir analysis")
  system("cp ../BCC2/*.in .")  #warning, this may wipe out edits made to leap.in ... only do this block on a new folder
  system("cp ../BCC2/*.trajin .") 
  system("cp ../BCC2/*.cpptraj .") 
}
Sys.setenv(AMBERHOME= "/home/radivot/soft/amber22")
system("echo $AMBERHOME")
system("echo $PATH")
system("$AMBERHOME/bin/tleap -s -f leapD13.in > leap.out")
...
system(s) #"grep 5592 ../*.out"
# system("$AMBERHOME/bin/cpptraj TC5b.prmtop <measure_trp_angles.cpptraj")
271000/500 # frame 542 in file 2

setwd("/home/radivot/GH/protein-folding-qc/res/tom/amber/DIA13") 
system("$AMBERHOME/bin/cpptraj TC5b.prmtop < extract_frame2.trajin > extract_frame2.out")

system("vmd lowest_energy_struct.pdb")  #terrible, stick with BCC2
...
system("$AMBERHOME/bin/cpptraj TC5b.prmtop <average_5_crd.cpptraj")
system("vmd equil_average_5.pdb")  ## same (still bad) # so far local averages have never helped


# system("$AMBERHOME/bin/cpptraj TC5b.prmtop <measure_trp_angles.cpptraj") # no need for this unless structure is right
