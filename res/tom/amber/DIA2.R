system("pwd")
setwd("/home/radivot/GH/protein-folding-qc/res/tom/amber/DIA2") 
system("pwd")
if (0){  # stuff to do just once, to initialize the folder
  system("mkdir analysis")
  system("cp ../trp19/*.in .")  #warning, this may wipe out edits made to leap.in ... only do this block on a new folder
  system("cp ../trp19/*.trajin .") 
  system("cp ../trp19/*.cpptraj .") 
}
Sys.setenv(AMBERHOME= "/home/radivot/soft/amber22")
system("echo $AMBERHOME")
system("echo $PATH")
system("$AMBERHOME/bin/tleap -s -f leapD2.in > leapD2.out")
...
d%>%ggplot(aes(x=t,y=E))+geom_line()
d%>%filter(E==min(E)) 
system("grep 42749 ../*.out")
1349500/500 # frame 2699 in file 9

setwd("/home/radivot/GH/protein-folding-qc/res/tom/amber/DIA2") 
system("$AMBERHOME/bin/cpptraj TC5b.prmtop < extract_frame9.trajin > extract_frame9.out")

system("vmd lowest_energy_struct.pdb")  ## bad, out of cage
...
system("$AMBERHOME/bin/cpptraj TC5b.prmtop <average_22-32_crd.cpptraj")
system("$AMBERHOME/bin/cpptraj TC5b.prmtop <measure_trp_angles.cpptraj")

system("vmd equil_average_22-32.pdb")  ## still bad, out of cage
