...
setwd(base)
diaS=c(2,3,4,5,6,7,8,9)
lat="DIA"
k=1
for (k in 1:length(bccS)) {
  ...
  system("mpirun -np 12 $AMBERHOME/bin/sander.MPI -O -i ../../amber/inputs/min1.in -o min1.out -p chig.prmtop -c chig.rst7 -r min1.ncrst")
  system.time(system("mpirun -np 12 $AMBERHOME/bin/sander.MPI -O -i ../../amber/inputs/heat1.in -p chig.prmtop -c min1.ncrst -r heat1.ncrst -o heat1.out -x heat1.nc"))
  for (i in 2:7)
    print(system.time(system(paste0("mpirun -np 12 $AMBERHOME/bin/pmemd.MPI -O -i ../../amber/inputs/heat",i,".in -p chig.prmtop -c heat",i-1,".ncrst -r heat",i,".ncrst -o heat",i,".out -x heat",i,".nc"))))
  ...
  system("$AMBERHOME/bin/cpptraj chig.prmtop < ../inputs/rmsd.trajin > rmsd.out")
