...
# 4.4 is length of unit cell

x$conect <- conect(x,safety=3.4) # a numeric value used to extend the atomic radii (=> radii are 0.2)
x$conect

visualize(x)
# write.pdb(x,"trpDia/pdbs/LatFCC.pdb")

# before 9A x 6 was 54 A across 6x6x6 cube, here go with 15*4.4=66 at end
...
# y$conect <- conect(y,safety=3.9) # if so this is really 4.4, which connects the corners 
# y$conect <- conect(y,safety=3.85) # still works, 3.86 is failure point for bonding corners
visualize(y)
write.pdb(y,"pdbs/BCC/LatBCC.pdb")
write_tsv(y$conect,"pdbs/BCC/conBCC.txt",col_names=F)
