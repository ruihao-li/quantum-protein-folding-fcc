...
run(session, 'close')
run(session, 'open ../TC5b/pdbs/FCC/LatFCC.pdb')
run(session, 'open pdbs/4qxx_CA.pdb')
...
run(session, 'size #1:2661,2662,2663 atomRadius 0.75')
run(session, 'size #1:1 atomRadius 0.5')
run(session, 'align #2:1-3 to #1:2661,2662,2663 move chains')
run(session, 'view #1:2661')


S=list()
f = open("../TC5b/pdbs/FCC/conFCC.txt", "r")
import re
for x in f:
  y=x.strip()
...
R=[0 for J in range(2,5)]
B1=[0 for J in range(2,5)]
for J in range(2,5):
  print("J is:",J)
