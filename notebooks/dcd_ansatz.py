...
def dig_count_ansatz(pauli_op):
    ...
    for label, coeff in zip(pauli_op.paulis.to_labels(), pauli_op.coeffs):
        # Identify indices of each Pauli type
        ...
        # Apply RX, RY, or RZ gates for single-qubit terms
        if len(x_indices) == 1 and len(y_indices) == 0 and len(z_indices) == 0:
            theta = Parameter(f"theta_X_{x_indices[0]}")
            qc.rx(theta, x_indices[0])
        if len(y_indices) == 1 and len(x_indices) == 0 and len(z_indices) == 0:
            theta = Parameter(f"theta_Y_{y_indices[0]}")
            qc.ry(theta, y_indices[0])
        if len(z_indices) == 1 and len(x_indices) == 0 and len(y_indices) == 0:
            theta = Parameter(f"theta_Z_{z_indices[0]}")
            qc.rz(theta, z_indices[0])

        # Add custom two-qubit RYZ gates for 'Y' and 'Z' terms
        if len(y_indices) == 1 and len(z_indices) == 1 and len(x_indices) == 0:
            theta = Parameter(f"theta_RYZ_{y_indices[0]}_{z_indices[0]}")
            # Assuming RYZ gate is properly defined and added to Qiskit's gate library
